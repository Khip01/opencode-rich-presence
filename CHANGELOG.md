...
- Discord presence "stuck" after the LAST OpenCode instance exits with `/exit`: in some cases the worker's `clearActivity` and `client.destroy` both hang past their timeouts, leaving the worker alive and bound to the Discord IPC socket. v2.0.8-rc5 deliberately dropped the parent's SIGKILL-after-grace fallback (PID-reuse safety), so the worker became an orphan that never exited. While alive, the orphan kept the IPC socket bound and Discord kept displaying the last activity. Now the worker installs an internal 2s kill switch on shutdown that calls `process.exit(0)` self-targeted — safe (no PID-reuse concern from the parent's side) and guaranteed to release the IPC socket so Discord clears the activity on the abrupt disconnect.
- Worker shutdown handler's `clearActivity` retried once with a 500ms timeout before giving up. Combined with the kill switch above, the worker now reliably exits within ~2s of receiving the shutdown command even if the Discord side is unresponsive.
- `gracefulExit` (SIGINT/SIGTERM handler) kill switch bumped from 150ms to 1500ms so the 1s-timeout `clearActivity` call has time to land before the worker is force-exited.
