...
#   curl -fsSL https://raw.githubusercontent.com/Khip01/opencode-rich-presence/main/install.sh | ORP_VERSION=v3.1.5 bash
#
...
# script works with a bare semver string (e.g. "3.1.5") and
...
if [ -z "$VERSION" ]; then
  ...
  if [ -z "$VERSION" ]; then
    ...
    err "Pin a version explicitly: ORP_VERSION=v3.1.5 bash install.sh"
...
# `github.ref_name` for a tag like `v3.1.5` is
# `v3.1.5` (the `v` is part of the ref). So the tarball
